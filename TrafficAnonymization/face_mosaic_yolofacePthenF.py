# -*- coding: utf-8 -*-
# video_mosaic_face_framewise_cascade.py
# Function: First detect person, then detect face within person ROI, frame-by-frame direct mosaic (no smoothing, no tracking, no TTA)
...
def process_video(
    ...
    while True:
        ...
        # 3) Frame-by-frame mosaic (no smoothing/retention)
...
# ----------------- Parameters -----------------
def parse_args():
    ap = argparse.ArgumentParser("Framewise Face Mosaic (Cascade: person->face, no smoothing)")
    ap.add_argument("--videos_dir", type=str, default="face_included_videos", help="Input video directory")
    ap.add_argument("--out_dir", type=str, default="face_mosaic_videos_yoloPF2", help="Output directory")
    ...
    ap.add_argument("--person_conf", type=float, default=0.35, help="Person detection confidence threshold")
    ap.add_argument("--person_iou", type=float, default=0.6, help="Person NMS IoU")
    ap.add_argument("--min_person_px", type=int, default=32, help="Minimum person width/height in pixels")
    ap.add_argument("--person_expand", type=float, default=0.18, help="Person ROI expansion ratio")
    ap.add_argument("--face_conf", type=float, default=0.30, help="Face detection confidence threshold")
    ap.add_argument("--face_iou", type=float, default=0.6, help="Face NMS IoU")
    ap.add_argument("--face_expand", type=float, default=0.45, help="Face box expansion ratio")
    ap.add_argument("--upscale", type=float, default=1.8, help="ROI upsampling ratio before face detection")
    ap.add_argument("--mosaic_block", type=int, default=15, help="Mosaic block size")
